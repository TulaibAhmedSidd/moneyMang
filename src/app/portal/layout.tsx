...
function PortalNavContent({
  ...
  const { prefetchTab } = usePortalData();

  const navItems = [
    { name: "Dashboard", href: "/portal", icon: "📊", tabKey: "dashboard" as const },
    { name: "Transactions", href: "/portal/transactions", icon: "💸", tabKey: "transactions" as const },
    { name: "Analytics", href: "/portal/analytics", icon: "📈", tabKey: "analytics" as const },
    { name: "Calendar", href: "/portal/calendar", icon: "📅", tabKey: "calendar" as const },
    { name: "Settings", href: "/portal/settings", icon: "⚙️", tabKey: "settings" as const },
  ];
    ...
    <div className="flex h-screen w-screen flex-col md:flex-row overflow-hidden bg-zinc-950 text-slate-100 font-sans">
      ...
      <aside className="hidden md:flex w-64 flex-col border-r border-zinc-800/80 bg-zinc-900/40 backdrop-blur-xl">
        <div className="flex h-16 items-center px-6 border-b border-zinc-800/80 gap-3">
          <div 
            className="flex h-8 w-8 items-center justify-center rounded-lg font-bold text-white shadow-md"
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.name}
                href={item.href}
                onMouseEnter={() => prefetchTab(item.tabKey)}
                onTouchStart={() => prefetchTab(item.tabKey)}
...
export default function PortalLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <PortalDataProvider>
      <PortalNavContent user={user} handleLogout={handleLogout} themeColor={themeColor}>
        {children}
      </PortalNavContent>
    </PortalDataProvider>
  );
